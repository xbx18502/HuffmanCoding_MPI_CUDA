...
__device__ unsigned char d_bitSequenceGlobalMemory[256][255];
void lauchCUDAHuffmanCompress(unsigned char *inputFileData,
                              unsigned int *compressedDataOffset,
                              unsigned int inputFileLength, int numKernelRuns,
  ...
  if (integerOverflowFlag == 0) {
    ...
    if (numKernelRuns == 1) {
      createDataOffsetArray(compressedDataOffset, inputFileData,
                            inputFileLength);
    }
    ...
    // multiple run of kernel due to larger file or smaller gpu memory
      ...
      gpuBitPaddingFlag =
          (unsigned int *)calloc(numKernelRuns, sizeof(unsigned int));
      gpuMemoryOverflowIndex =
          (unsigned int *)calloc(numKernelRuns * 2, sizeof(unsigned int));
      createDataOffsetArray(compressedDataOffset, inputFileData,
                            inputFileLength, gpuMemoryOverflowIndex,
                            gpuBitPaddingFlag, mem_req);
    }
  }
  ...
  // when there is integer over flow
    ...
    if (numKernelRuns == 1) {
      ...
      integerOverflowIndex =
          (unsigned int *)calloc(numKernelRuns * 2, sizeof(unsigned int));
      createDataOffsetArray(compressedDataOffset, inputFileData,
                            inputFileLength, integerOverflowIndex,
                            bitPaddingFlag, 10240);
    }
    ...
    // overflow occurs and multiple run
      ...
      createDataOffsetArray(compressedDataOffset, inputFileData,
                            inputFileLength, integerOverflowIndex,
                            bitPaddingFlag, gpuMemoryOverflowIndex,
                            gpuBitPaddingFlag, 10240, mem_req);
    }
  }
  ...
  // GPU initiation
  {
    ...
    error = cudaMalloc((void **)&d_inputFileData,
                       inputFileLength * sizeof(unsigned char));
    if (error != cudaSuccess)
      printf("erro_1: %s\n", cudaGetErrorString(error));

    error = cudaMalloc((void **)&d_compressedDataOffset,
                       (inputFileLength + 1) * sizeof(unsigned int));
    if (error != cudaSuccess)
      printf("erro_2: %s\n", cudaGetErrorString(error));
    error =
        cudaMalloc((void **)&d_huffmanDictionary, sizeof(huffmanDictionary));
    if (error != cudaSuccess)
      printf("erro_3: %s\n", cudaGetErrorString(error));

    // memory copy input data, offset information and dictionary
    error = cudaMemcpy(d_inputFileData, inputFileData,
                       inputFileLength * sizeof(unsigned char),
                       cudaMemcpyHostToDevice);
    if (error != cudaSuccess)
      printf("erro_4: %s\n", cudaGetErrorString(error));
    error = cudaMemcpy(d_compressedDataOffset, compressedDataOffset,
                       (inputFileLength + 1) * sizeof(unsigned int),
                       cudaMemcpyHostToDevice);
    if (error != cudaSuccess)
      printf("erro_5: %s\n", cudaGetErrorString(error));
    error = cudaMemcpy(d_huffmanDictionary, &huffmanDictionary,
                       sizeof(huffmanDictionary), cudaMemcpyHostToDevice);
    if (error != cudaSuccess)
      printf("erro_6: %s\n", cudaGetErrorString(error));
    ...
    if (constMemoryFlag == 1) {
      //   error =
      //       cudaMemcpyToSymbol(d_bitSequenceConstMemory,
  ...
  if (numKernelRuns == 1) {
    // no overflow
    if (integerOverflowFlag == 0) {
      error = cudaMalloc((void **)&d_byteCompressedData,
      ...
      error = cudaMemset(d_byteCompressedData, 0,
                         compressedDataOffset[inputFileLength] *
                             sizeof(unsigned char));
      if (error != cudaSuccess)
        printf("erro_8: %s\n", cudaGetErrorString(error));
      ...
      // run kernel
      compress<<<1, block_size>>>(d_inputFileData, d_compressedDataOffset,
                                  d_huffmanDictionary, d_byteCompressedData,
      ...
      error = cudaMemcpy(inputFileData, d_inputFileData,
                         ((compressedDataOffset[inputFileLength] / 8)) *
                             sizeof(unsigned char),
    ...
    // integer overflow occurs
      ...
      error = cudaMalloc((void **)&d_byteCompressedData,
                         compressedDataOffset[integerOverflowIndex[0]] *
                             sizeof(unsigned char));
      if (error != cudaSuccess)
        printf("erro_7: %s\n", cudaGetErrorString(error));
      error = cudaMalloc((void **)&d_byteCompressedDataOverflow,
                         compressedDataOffset[inputFileLength] *
                             sizeof(unsigned char));
      if (error != cudaSuccess)
        printf("erro_8: %s\n", cudaGetErrorString(error));
      ...
      if (error != cudaSuccess)
        printf("erro_9: %s\n", cudaGetErrorString(error));
      error = cudaMemset(d_byteCompressedDataOverflow, 0,
                         compressedDataOffset[inputFileLength] *
                             sizeof(unsigned char));
      if (error != cudaSuccess)
        printf("erro_10: %s\n", cudaGetErrorString(error));
      ...
      compress<<<1, block_size>>>(d_inputFileData, d_compressedDataOffset,
                                  d_huffmanDictionary, d_byteCompressedData,
                                  d_byteCompressedDataOverflow, inputFileLength,
      ...
      if (bitPaddingFlag[0] == 0) {
        error = cudaMemcpy(inputFileData, d_inputFileData,
                           (compressedDataOffset[integerOverflowIndex[0]] / 8) *
                               sizeof(unsigned char),
                           cudaMemcpyDeviceToHost);
        if (error != cudaSuccess)
          printf("erro_11: %s\n", cudaGetErrorString(error));
        error = cudaMemcpy(
            ...
            &d_inputFileData[(compressedDataOffset[integerOverflowIndex[0]] /
                              8)],
            ((compressedDataOffset[inputFileLength] / 8)) *
                sizeof(unsigned char),
            cudaMemcpyDeviceToHost);
    ...
    if (integerOverflowFlag == 0) {
      error = cudaMalloc((void **)&d_byteCompressedData,
                         (compressedDataOffset[gpuMemoryOverflowIndex[1]]) *
                             sizeof(unsigned char));
      if (error != cudaSuccess)
        printf("erro_7: %s\n", cudaGetErrorString(error));
      ...
      for (i = 0; i < numKernelRuns; i++) {
        ...
        compress<<<1, block_size>>>(
            d_inputFileData, d_compressedDataOffset, d_huffmanDictionary,
            d_byteCompressedData, gpuMemoryOverflowIndex[i * 2],
            constMemoryFlag, gpuMemoryOverflowIndex[i * 2 + 1]);
        cudaError_t error_kernel = cudaGetLastError();
        if (error_kernel != cudaSuccess)
          printf("erro_final: %s\n", cudaGetErrorString(error_kernel));
        ...
        if (gpuBitPaddingFlag[i] == 0) {
          error = cudaMemcpy(
              &inputFileData[pos], d_inputFileData,
          ...
          error = cudaMemcpy(
              &inputFileData[pos - 1], d_inputFileData,
              ((compressedDataOffset[gpuMemoryOverflowIndex[i * 2 + 1]] / 8)) *
                  sizeof(unsigned char),
              cudaMemcpyDeviceToHost);
          if (error != cudaSuccess)
          ...
          pos +=
              (compressedDataOffset[gpuMemoryOverflowIndex[i * 2 + 1]] / 8) - 1;
        }
      }
      ...
      for (i = 0; i < numKernelRuns; i++) {
        if (integerOverflowIndex[i] != 0) {
          ...
          compress<<<1, block_size>>>(
              d_inputFileData, d_compressedDataOffset, d_huffmanDictionary,
              d_byteCompressedData, d_byteCompressedDataOverflow,
          ...
          if (gpuBitPaddingFlag[i] == 0) {
            if (bitPaddingFlag[i] == 0) {
              ...
              error = cudaMemcpy(
                  ...
                  &d_inputFileData[(
                      compressedDataOffset[integerOverflowIndex[i]] / 8)],
                  ((compressedDataOffset[gpuMemoryOverflowIndex[i * 2 + 1]] /
                    8)) *
                      sizeof(unsigned char),
                  cudaMemcpyDeviceToHost);
              if (error != cudaSuccess)
              ...
              if (error != cudaSuccess)
                printf("erro_11: %s\n", cudaGetErrorString(error));
              unsigned char temp_compByte = inputFileData
            ...
            if (bitPaddingFlag[i] == 0) {
              ...
              if (error != cudaSuccess)
                printf("erro_12: %s\n", cudaGetErrorString(error));
              inputFileData[pos - 1] =
                  temp_gpuCompByte | inputFileData[pos - 1];
              pos += (compressedDataOffset[integerOverflowIndex[i]] / 8) +
                     (compressedDataOffset[gpuMemoryOverflowIndex[i * 2 + 1]] /
                      8) -
                     1;
              ...
              if (error != cudaSuccess)
                printf("erro_12: %s\n", cudaGetErrorString(error));
              inputFileData[(compressedDataOffset[pos - 1 +
          ...
          if (gpuBitPaddingFlag[i] == 0) {
            ...
            if (error != cudaSuccess)
              printf("erro_12: %s\n", cudaGetErrorString(error));
            inputFileData[pos - 1] =
                temp_huffmanTreeNode | inputFileData[pos - 1];
            pos +=
                (compressedDataOffset[gpuMemoryOverflowIndex[i * 2 + 1]] / 8) -
                1;
          }
